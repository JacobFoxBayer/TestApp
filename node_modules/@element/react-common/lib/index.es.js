import React, { createContext, useLayoutEffect, useEffect, useRef } from 'react';
...
const HoistedContext = /*#__PURE__*/createContext({});
...
const addProps = (children, props) => {
  ...
  return React.Children.map(children, (child, index) => {
    ...
    if (! /*#__PURE__*/React.isValidElement(child)) {
      return /*#__PURE__*/React.createElement('div', propsActual, child);
    }
    return /*#__PURE__*/React.cloneElement(child, propsActual);
  });
};
...
const edsRandomId = name => {
  if (name) return `eds-${`${name}-`}${nanoid(5)}`;
  return `eds-${nanoid(5)}`;
};
...
export { ConditionalWrapper, HoistedContext as HoistingContext, addClass, addProps, deprecatedComponent, deprecatedOneOf, deprecatedProp, edsQuerySelector, edsRandomId, edsWarning, formatUnit, getElementNotFoundWarning, getInvalidSelectorWarning, hoistingEnabled, iconList, isSelectorValid, queryCheck, requiredIfPropType, returnFocusAfterClosing, useIsomorphicLayoutEffect, usePrevious };

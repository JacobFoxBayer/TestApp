...
var updateToken = function updateToken(route, path) {
  ...
  var ttl = namespace.get('auth.expires', 0);
  ...
  if (cookieToken) {
    ...
    namespace.set('auth.token', cookieToken); // fetch token in background to get expiration.
...
var authTokenRequest = function authTokenRequest(route, path) {
  ...
  return agent.get(url).then(function (_ref) {
    ...
    var ttl = getTime() + expiresIn * 1000 - 5 * 1000 * 60;
    namespace.set('authHeader', function () {
      return {
    ...
    namespace.set('auth.expires', ttl); // refreshing can be disabled for testing systems that wait for the event
    // loop to idle.
...
var createPromise = function createPromise(options) {
  if (settings.authEnabled()) {
    debug.log('enableAuthTokenSupport: auth is enabled'); // eslint-disable-next-line no-restricted-globals
